...
export function FAQClient({ faqData }: FAQClientProps) {
  ...
  // Build FAQ items from Supabase data
  const faqItems = React.useMemo(() => {
    return faqData.map((item) => ({
      id: item.id,
      category: item.category,
      question: locale === "ru" ? item.question_ru : item.question_ro,
      answer: locale === "ru" ? item.answer_ru : item.answer_ro,
      keywords: [
        item.question_ro.toLowerCase(),
        item.question_ru.toLowerCase(),
        item.answer_ro.toLowerCase(),
        item.answer_ru.toLowerCase(),
      ],
    }));
  }, [faqData, locale]);
