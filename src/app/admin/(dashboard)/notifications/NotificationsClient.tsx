...
import { Bell, Send, Smartphone, Loader2, CheckCircle, AlertCircle, FlaskConical } from "lucide-react";
...
type Audience = "all" | "emails";

interface NotificationsClientProps {
  adminEmail: string;
}

export function NotificationsClient({ adminEmail }: NotificationsClientProps) {
  ...
  function handleTestToSelf() {
    if (!adminEmail) return;
    setAudience("emails");
    setEmailsRaw(adminEmail);
  }
  ...
  async function handleSend() {
    ...
    if (audience === "emails") {
      targetEmails = parseEmails(emailsRaw);
      if (targetEmails.length === 0) {
        setResult({ success: false, message: "Укажите хотя бы один email" });
        return;
      }
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        const audienceLabel = targetEmails
          ? ` (${targetEmails.length} ${targetEmails.length === 1 ? "пользователю" : "пользователям"})`
          : "";
        setResult({
          success: true,
          message: `Отправлено: ${data.sent ?? 0} устройств${audienceLabel}`,
        });
        setTitleRo("");
        setBodyRo("");
        setTitleRu("");
        ...
        setType("general");
        setActionRoute("none");
        // Audience and emails оставляем — удобно отправлять серию тестов подряд.
        loadStats();
      } else {
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-gray-700">Получатели</p>
                {adminEmail && (
                  <button
                    type="button"
              ...
              >
                <label
                  ...
                  className={`flex items-center gap-2 rounded-lg border p-3 cursor-pointer transition-colors ${
                    audience === "all" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:bg-gray-50"
                  }`}
                >
                  <RadioGroupItem value="all" id="audience-all" />
                  <span className="text-sm font-medium">Всем</span>
                </label>
                <label
                  htmlFor="audience-emails"
                  className={`flex items-center gap-2 rounded-lg border p-3 cursor-pointer transition-colors ${
                    audience === "emails" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:bg-gray-50"
                  }`}
                >
                  <RadioGroupItem value="emails" id="audience-emails" />
              ...
              {audience === "emails" && (
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-gray-400">
                    Через запятую, пробел или с новой строки. Получат пользователи с этими email,
                    привязанные к device tokens.
                  </p>
                </div>
              )}
            </div>
            ...
            >
              {sending ? (
                ...
                <>
                  ...
                  {audience === "emails" ? "Отправить выбранным" : "Отправить всем"}
                </>
              )}
            </Button>
          </CardContent>
